    ...
    def train_epoch(self, data):
        ...
        try:
            ...
            if isinstance(data, dict):
                if 'text' in data:
                    ...
                    if self.language_trainer:
                        ...
                        if not isinstance(text, str):
                            text = str(text)
                        input_tensor = torch.tensor([ord(c) for c in text[:768]]).float()
                        # Pad if necessary to reach input_dim
                        if input_tensor.size(0) < 768:
                            input_tensor = F.pad(input_tensor, (0, 768 - input_tensor.size(0)))
                        # Truncate if too long
                        elif input_tensor.size(0) > 768:
                            input_tensor = input_tensor[:768]
                elif 'story' in data:
                    # Use story field as text if available
                    text = data.get('story', '')
                    if not isinstance(text, str):
                        text = str(text)
                    input_tensor = torch.tensor([ord(c) for c in text[:768]]).float()
                    ...
                    # Convert general dictionary to tensor
                    # Extract symbol, emotion, breath, paradox if available
                    symbol = str(data.get('symbol', ''))
                    emotion = str(data.get('emotion', ''))
                    breath = str(data.get('breath', ''))
                    paradox = str(data.get('paradox', ''))
                    
                    # Concatenate fields
                    text = f"{symbol} {emotion} {breath} {paradox}"
                    # Encode as character tensor
                    input_tensor = torch.tensor([ord(c) for c in text[:768]]).float()
                    # Pad if necessary to reach input_dim
                    if input_tensor.size(0) < 768:
                        input_tensor = F.pad(input_tensor, (0, 768 - input_tensor.size(0)))
                ...
                # Direct string input
                input_tensor = torch.tensor([ord(c) for c in data[:768]]).float()
                # Pad if necessary
                if input_tensor.size(0) < 768:
                    input_tensor = F.pad(input_tensor, (0, 768 - input_tensor.size(0)))
            else:
                ...
                try:
                    data_str = str(data)
                    input_tensor = torch.tensor([ord(c) for c in data_str[:768]]).float()
                    # Pad if necessary
                    if input_tensor.size(0) < 768:
                        input_tensor = F.pad(input_tensor, (0, 768 - input_tensor.size(0)))
                except:
            ...
            encoded = self.encoder(input_tensor.unsqueeze(0))  # Add batch dimension
            
            # Add positional encoding and reshape for transformer
            pos_encoded = encoded  # Already has batch dimension
            
            # Forward pass through transformer
            transformed = self.transformer(pos_encoded)
            
            # Forward pass through decoder (keep batch dimension)
            decoded = self.decoder(transformed)
            
            # Process through subnets (using the squeezed tensor)
            decoded_nobatch = decoded.squeeze(0)
            math_result = self.mathematics_subnet(decoded_nobatch)
            lang_result = self.language_subnet(decoded_nobatch)
            
            # Prepare input for physics subnet
            physics_input = {"quantum_state": decoded_nobatch.clone()}
            physics_result = self.physics_subnet(physics_input)
            
            # Calculate reconstruction loss - need to match dimensions
            # Compare the output to the original input properly
            reconstruction_loss = F.mse_loss(decoded.squeeze(0), original_input)
            
            # Extract math and language losses
            ...
            for k, v in physics_result.items():
                if isinstance(v, torch.Tensor) and v.requires_grad:
                    physics_loss += torch.mean(v)
                    
            # Convert to tensor if it's still a float
            if isinstance(physics_loss, float):
                physics_loss = torch.tensor(physics_loss, requires_grad=True)
            ...
            # Calculate metrics
            with torch.no_grad():
                quantum_fields = self._calculate_quantum_fields(decoded_nobatch)
                topology = self._analyze_topological_structures(decoded_nobatch)
                resonance = self._calculate_harmonic_resonance(decoded_nobatch)
                cycles = self._extract_cycles(decoded_nobatch)
