...
class SystemMetricsThread(QThread):
    ...
    def stop(self):
        """Stop the metrics thread gracefully"""
        self._running = False
...
class VersionDataManager(QObject):
    ...
    def _auto_save(self):
        ...
        try:
            ...
            # Save current state to both file system and database
                ...
                if self.db_manager:
                    try:
                        state_data = {
                            "id": timestamp,
                            "timestamp": time.time(),
                            "state_type": "auto_save",
                            "metrics": self.parent().current_metrics if self.parent() else {},
                            "version_data": self.version_summary.get(timestamp, {})
                        }
                        self.db_manager.save_neural_state(state_data)
                        logger.info(f"Auto-save state stored in main database: {timestamp}")
                    except Exception as e:
                        logger.error(f"Error saving to main database: {str(e)}")
    ...
    def save_current_version(self):
        ...
        try:
            ...
            model_state = self.parent().get_model_state() if self.parent() else {}
            metrics = self.parent().current_metrics if self.parent() else {}
            ...
            if self.db_manager:
                try:
                    state_data = {
                        "id": timestamp,
                        "timestamp": time.time(),
                        "state_type": "manual_save",
                        "metrics": metrics,
                        "version_data": metadata
                    }
                    self.db_manager.save_neural_state(state_data)
                    logger.info(f"Version state stored in main database: {timestamp}")
                except Exception as e:
                    logger.error(f"Error saving to main database: {str(e)}")
...
class CentralNodeMonitor(QObject):
    ...
    def __init__(self, parent=None):
        ...
        # Initialize versions attribute
        self.versions = {}  # Dictionary to store version information
    ...
    def cleanup(self):
        ...
        try:
            if hasattr(self, 'metrics_thread') and self.metrics_thread:
                ...
                logger.info("Successfully stopped metrics thread")
        except Exception as e:
    ...
    def get_current_state(self):
        """Get the current state of the Central Node Monitor."""
        try:
            state = {
                'versions': self.versions,
                'metrics': self.get_metrics(),
                'quantum_state': self.get_quantum_state(),
                'cosmic_state': self.get_cosmic_state(),
                'timestamp': time.time()
            }
            return state
    ...
    def get_quantum_state(self):
        ...
        try:
            if not hasattr(self, 'quantum_manager'):
                return None
                
            return {
                'field_strength': self.quantum_manager.get_field_strength(),
                'entangled_nodes': self.quantum_manager.get_entangled_nodes(),
                'phase': self.quantum_manager.get_phase(),
                'frequency': self.quantum_manager.get_frequency()
            }
    ...
    def get_cosmic_state(self):
        """Get the current cosmic state information."""
        try:
            if not hasattr(self, 'cosmic_manager'):
                return None
                
            return {
                'field_strength': self.cosmic_manager.get_field_strength(),
                'dimensional_resonance': self.cosmic_manager.get_resonance(),
                'universal_phase': self.cosmic_manager.get_phase(),
                'cosmic_frequency': self.cosmic_manager.get_frequency()
            }
