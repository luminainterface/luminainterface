...
async def test_system_growth():
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8710") as client:
        ...
        initial_growth_events = initial_stats["model_stats"]["growth_events"]
        ...
        for concept_id, label, drift_type in concepts:
            ...
            response = await client.post(
                ...
                json={
                    "concept_id": concept_id,
                    "vectors": drifted_embeddings.tolist(),
                    "labels": [label] * 20,
                    "metadata": { "drift_type": drift_type, "is_drifted": True },
                    "timestamp": datetime.now().isoformat()
                }
            )
            assert response.status_code == 200
            ...
            # Get concept status to check drift
            response = await client.get(f"/api/v1/model/status/{concept_id}")
            assert response.status_code == 200
            status = response.json()
            drift = status["metrics"].get("drift", 0.0)
            
            if drift > 0.001:  # Using drift threshold
                print(f"\nDrift {drift} exceeds threshold 0.001, attempting growth...")
                
                # Get current layer sizes
                response = await client.get("/api/v1/model/health")
                assert response.status_code == 200
                health = response.json()
                layer_sizes = health["model_stats"]["layer_sizes"]
                ...
                print(f"\nDrift {drift} below threshold 0.001, skipping growth")
        
        # Get final model health
        response = await client.get("/api/v1/model/health")
        assert response.status_code == 200
        ...
        assert final_stats["model_stats"]["growth_events"] > initial_growth_events, "No new growth events occurred"
        
        # Check if any layer grew
        layers_grew = False
        for i in range(len(initial_stats["model_stats"]["layer_sizes"])):
            if final_stats["model_stats"]["layer_sizes"][i] > initial_stats["model_stats"]["layer_sizes"][i]:
                layers_grew = True
                break
        assert layers_grew, "No layers grew in size"
        
        print("\n=== Growth Verification ===")
        print(f"Initial layer sizes: {initial_stats['model_stats']['layer_sizes']}")
        print(f"Final layer sizes: {final_stats['model_stats']['layer_sizes']}")
        print(f"Initial growth events: {initial_growth_events}")
        print(f"Final growth events: {final_stats['model_stats']['growth_events']}")
        print(f"Concepts tracked: {final_stats['model_stats']['concepts_tracked']}")
