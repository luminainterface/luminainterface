...
class NPUBackgroundWorker:
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Initialize Redis connection with improved URL parsing
            ...
            if self.redis_url.startswith("redis://"):
                ...
                # Get password from environment if not in URL
                password = os.getenv('REDIS_PASSWORD', '02211998')
    ...
    async def stop(self):
        ...
        # Unregister worker
        await self.redis_client.hdel("npu_background_workers", self.worker_id)
    ...
    async def _process_analysis_task(self, task: BackgroundTask) -> bool:
        ...
        try:
            ...
            await self.redis_client.setex(result_key, 3600, json.dumps(result))
        ...
        while self.is_running:
            try:
                for target in crawl_targets:
                    ...
                    last_crawl = await self.redis_client.get(f"last_crawl:{url_hash}")
    ...
    async def _add_background_task(self, task_type: str, data: Dict[str, Any], priority: int = 5):
        ...
        await self.redis_client.xadd(stream_name, task_data)
